...
(function($) {
	...
	$.fn.photoTag = function( options ){
		...
		var defaultOptions = {
			...
			addTagUrl: 'photo_tag_phps/add-tag.php',
			parametersForNewTag: {
				name: {
					parameterKey: 'name',
					isAutocomplete: true,
					autocompleteUrl: 'photo_tag_phps/names.php',
					label: '',
					placeholder: "Kıyafet Adı Giriniz"
				},
				brand: {
					parameterKey: 'brand',
					isAutocomplete: true,
			...
			literals:{
				communicationProblem: 'Tag Kayıt Edilemedi. Yeniden Deneyiniz!',
				saveTag: 'Kaydet',
				cancelTag: 'İptal',
				addNewTag: "Kıyafet Tag'le",
				removeTag: "Kaldır"
			},
			tag: {
...
/*			tagBox.mouseover(
		...
		var createNewTagForm = function( tempTagBox, image, image_id ){
			...
			$.each(options.parametersForNewTag,function( i, properties ){
				var input = $('<input placeholder="'+ properties.placeholder +'" type="text" autocomplete="off" id="tempInput_'+i+'" name="'+properties.parameterKey+'">');
		...
		var createAddTagLink = function( image, image_id ){
			var addTagLink = $('<a id="'+ options.imageWrapBox.addNewLinkIdPrefix + image_id + '" href="#" class="tag_an_aski">'+ options.literals.addNewTag +'</a>');
		...
		var extendTagBoxAttributes = function( tagBox, tagJSON, image, image_id ){
			if(options.tag.flashAfterCreation){
				...
				if(!$.browser.msie)
					$(tagBox).stop().animate({ opacity: 1.0 }, 800);
				else
					$(tagBox).css({ opacity: 1.0 });
			};
			if(options.imageWrapBox.showTagList){
